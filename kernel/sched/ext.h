...
enum scx_enq_flags {
	 ...
	 * %SCX_OPS_ENQ_LAST_TASK is specified, they're ->enqueue()'d with
	 * %SCX_ENQ_LAST_TASK flag.
	 *
	 * If the bpf scheduler wants to continue executing the task,
	 * ->enqueue() should dispatch the task to %SCX_DQ_LOCAL immediately.
	 * If the task gets queued on a different dq or the bpf side, the bpf
	 * scheduler is responsible for triggering a follow-up scheduling event.
	 * Otherwise, Execution may stall.
	 */
	SCX_ENQ_LAST_TASK	= 1LLU << 32,

	SCX_ENQ_WAKE_IDLE	= 1LLU << 33,	// XXX - actual wakeup not implemented yet
	SCX_ENQ_CLEAR_OPSS	= 1LLU << 40,	/* internal */
};
...
static inline void task_dead_scx(struct task_struct *p) {}
static inline void post_schedule_scx(struct rq *rq) {}
