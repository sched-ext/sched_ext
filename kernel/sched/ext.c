			    ...
			    int sticky_cpu)
{
	...
	if (scx_ops_enabled(p, SCX_OPSEN_EXCLUDE_DISABLING)) {
		if (sticky_cpu == cpu_of(rq) || unlikely(!rq->online)) {
			dq_id = SCX_DQ_LOCAL;
		} else {
		     ...
		     struct task_struct *p)
{
	 ...
	 * between scx_bpf_dispatch() calls and here. If @p is destined to an
	 * offline CPU, tell the caller to dump it into the global dq. As this
	 * is an allowed behavior, don't trigger an ops error.
	 */
