...
static DEFINE_PER_CPU(cpumask_var_t, cpus_to_wakeup);
#endif
						   ...
						   struct task_struct *p)
{
	...
	if (unlikely(!dq)) {
		if (dq_id < 0)
			scx_ops_error("got error verdict %lld for pid %d [%s]",
				      dq_id, p->pid, p->comm);
		else
			scx_ops_error("non-existent dq 0x%llx for pid %d [%s]",
				      dq_id, p->pid, p->comm);
		return &scx_dq_global;
	}
			    ...
			    int sticky_cpu)
{
	...
	if (scx_ops_enabled(p, SCX_OPSEN_EXCLUDE_DISABLING)) {
		if (sticky_cpu == cpu_of(rq)) {
			...
			if (enq_flags & SCX_ENQ_LAST_TASK) {
				if (SCX_HAS_OP(enqueue) &&
				    (scx_ops.flags & SCX_OPS_ENQ_LAST_TASK)) {
		...
		if (sticky_cpu == cpu_of(rq)) {
			dq_id = SCX_DQ_LOCAL;
		} else if (enq_flags & SCX_ENQ_LAST_TASK) {
			p->scx.slice = SCX_BASE_SLICE;
			dq_id = SCX_DQ_LOCAL;
		} else {
			dq_id = SCX_DQ_GLOBAL;
		}
		     ...
		     struct task_struct *p)
{
	...
	if (dq_id == SCX_DQ_LOCAL) {
		...
		if (!ops_cpu_valid(cpu)) {
			scx_ops_error("invalid cpu %d in SCX_DQ_LOCAL_ON verdict for pid %d [%s]",
				      cpu, p->pid, p->comm);
			return DTL_INVALID;
		}
		dst_rq = cpu_rq(cpu);
	...
	if (rq == src_rq && rq == dst_rq) {
		dispatch_enqueue(&dst_rq->scx.local_dq, p, SCX_ENQ_CLEAR_OPSS);
	...
	if (cpumask_test_cpu(cpu_of(dst_rq), p->cpus_ptr)) {
		...
		dispatch_to_local_dq_lock(rq, rf, src_rq, dst_rq);
		...
		if (src_rq == dst_rq) {
			...
			if (likely(dsp)) {
				p->scx.holding_cpu = -1;
				dispatch_enqueue(&dst_rq->scx.local_dq, p, 0);
			}
		} else {
		...
		dispatch_to_local_dq_unlock(rq, rf, src_rq, dst_rq);

		return dsp ? DTL_DISPATCHED : DTL_LOST;
	}
#endif /* CONFIG_SMP */

	scx_ops_error("SCX_DQ_LOCAL[_ON] verdict target cpu %d not allowed for pid %d [%s]",
		      cpu_of(dst_rq), p->pid, p->comm);
	return DTL_INVALID;
}
...
retry:
	...
	switch (dispatch_to_local_dq(rq, rf, dq_id, p)) {
	case DTL_DISPATCHED:
		return true;
	case DTL_LOST:
		return false;
	...
	dispatch_enqueue(dq, p, SCX_ENQ_CLEAR_OPSS);
	return false;
}
...
int balance_scx(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if (prev_on_scx) {
		 ...
		 * DISABLING test.
		 */
		if ((prev->scx.flags & SCX_TASK_QUEUED) && prev->scx.slice > 0 &&
		    scx_ops_enable_state() != SCX_OPS_DISABLING) {
...
static void put_prev_task_scx(struct rq *rq, struct task_struct *p)
{
	...
	if (p->scx.flags & SCX_TASK_QUEUED) {
		...
		if (list_empty(&rq->scx.local_dq.fifo))
			do_enqueue_task(rq, p, SCX_ENQ_LAST_TASK, -1);
		else
...
static struct task_struct *pick_next_task_scx(struct rq *rq)
{
	...
	if (unlikely(!p->scx.slice)) {
		if (scx_ops_enable_state() != SCX_OPS_DISABLING &&
			...
			printk_deferred(KERN_WARNING "sched_ext: %s(%d) has "
					"zero slice in pick_next_task_scx(), "
					"ops_enabled=%d\n",
...
static int scx_pick_idle_cpu(const struct cpumask *cpus_allowed)
{
	...
	do {
		...
		if (cpu < nr_cpu_ids) {
			cpumask_andnot(idle_masks.smt, idle_masks.smt,
...
static int scx_ops_prep_enable(struct task_struct *p, struct task_group *tg)
{
	if (SCX_HAS_OP(prep_enable)) {
		...
		return scx_ops.prep_enable(p, &args);
	}
...
void scx_post_fork(struct task_struct *p)
{
	...
	spin_lock(&scx_tasks_lock);
	list_add_tail(&p->scx.tasks_node, &scx_tasks);
	spin_unlock(&scx_tasks_lock);
...
void task_dead_scx(struct task_struct *p)
{
	if (p->scx.flags & SCX_TASK_OPS_ENABLED) {
		struct rq_flags rf;
		struct rq *rq;
...
void post_schedule_scx(struct rq *rq)
{
#ifdef CONFIG_SMP
	struct cpumask *cpumask = this_cpu_cpumask_var_ptr(cpus_to_wakeup);
	int cpu;
...
int scx_tg_online(struct task_group *tg)
{
	...
	if (SCX_HAS_OP(cgroup_init)) {
		ret = scx_ops.cgroup_init(tg->css.cgroup);
		if (!ret)
...
int scx_can_attach(struct task_group *tg, struct task_struct *p)
{
	...
	if (SCX_OP_ENABLED(cgroup_prep_move, p, 0)) {
		...
		return scx_ops.cgroup_prep_move(p, cgroup_move_from,
						tg_cgrp(tg));
	} else {
		return 0;
	}
...
void set_task_rq_scx(struct task_struct *p, int type)
{
	if (type == TASK_MOVE_CGROUP) {
		/*
...
void scx_group_set_weight(struct task_group *tg, unsigned long weight)
{
	...
	if (tg->scx_weight != weight) {
		if (SCX_HAS_OP(set_group_weight))
			scx_ops.set_group_weight(tg_cgrp(tg), weight);
...
DEFINE_SCHED_CLASS(ext) = {
	...
	/* XXX: gotta implement cpu on/offlining */
...
static int scx_cgroup_init(void)
{
	...
	css_for_each_descendant_pre(css, &root_task_group.css) {
		...
		ret = scx_ops.cgroup_init(css->cgroup);
		if (ret) {
			css_put(css);
			return ret;
		}
...
static s64 scx_ops_disabling_enqueue(struct task_struct *p, u64 enq_flags)
{
	if (enq_flags & SCX_ENQ_LAST_TASK)
		return SCX_DQ_LOCAL;
	else
...
static void scx_ops_disabling_consume(s32 cpu)
{
...
static void scx_ops_disable_workfn(struct kthread_work *work)
{
	 ...
	 */
	scx_ops.enqueue = scx_ops_disabling_enqueue;
	scx_ops.consume = scx_ops_disabling_consume;
...
forward_progress_guaranteed:
	...
	static_branch_disable(&scx_ops_all_enabled);
	...
	while ((p = scx_task_iter_next_filtered_locked(&sti))) {
		...
		sched_deq_and_put_task(p, DEQUEUE_SAVE, &ctx);
		scx_ops_disable_task(p);
		p->scx.slice = min_t(u64, p->scx.slice, SCX_BASE_SLICE);
		sched_enq_and_set_task(&ctx);
	}
	scx_task_iter_exit(&sti);
	spin_unlock(&scx_tasks_lock);
	...
	static_branch_enable(&scx_ops_all_disabled);
...
static int scx_ops_enable(struct sched_ext_ops *ops)
{
	...
	/*
	 * The synchronization is tricky because the first phase of disabling
	 * can't grab any mutex. See scx_ops_disable_workfn() for details.
	 */
	percpu_down_write(&scx_ops_rwsem);
	...
	static_branch_disable(&scx_ops_all_disabled);
	 ...
	 * have ops enabled and may need to be recovered.
	 */
	...
	/*
	 * Enable ops for each task. Fork is excluded by scx_ops_rwsem, so
	 * iterating tasks once is sufficient.
	 */
	spin_lock(&scx_tasks_lock);
	scx_task_iter_init(&sti);
	while ((p = scx_task_iter_next_filtered(&sti))) {
		...
		if (ret) {
			pr_err("sched_ext: prep_enable failed (%d) for pid %d [%s] while loading",
			       ret, p->pid, p->comm);
			put_task_struct(p);
			spin_lock(&scx_tasks_lock);
			break;
	...
	static_branch_enable(&scx_ops_all_enabled);
	percpu_up_write(&scx_ops_rwsem);
...
static const struct sysrq_key_op sysrq_sched_ext_reset_op = {
	...
	.action_msg	= "Resetting sched_ext to FIFO scheduling",
...
void __init init_sched_ext_class(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		rq->scx.nr_running = 0;
#ifdef CONFIG_SMP
		BUG_ON(!zalloc_cpumask_var(&per_cpu(cpus_to_wakeup, cpu),
					   GFP_KERNEL));
#endif
	}
 ...
 */
static __used noinline int scx_bpf_create_dq(s64 dq_id, int node)
{
 ...
 */
__used noinline int scx_bpf_dispatch(struct task_struct *p, s64 dq_id)
{
	int idx;
 ...
 * scx_ops operation and the actual kicking is performed asynchronously in
 * post_schedule_scx().
 */
static __used noinline void scx_bpf_kick_cpu(s32 cpu)
{
	if (!ops_cpu_valid(cpu)) {
...
#ifdef CONFIG_SMP
	cpumask_set_cpu(cpu, this_cpu_cpumask_var_ptr(cpus_to_wakeup));
#endif
}
...
int scx_bpf_pick_idle_cpu(const struct cpumask *cpus_allowed)
{
	if (!static_branch_likely(&scx_builtin_idle_enabled)) {
		scx_ops_error("built-in idle tracking is disabled");
		return -EBUSY;
 ...
 */
static __used noinline int scx_bpf_destroy_dq(s64 dq_id)
{
 ...
 */
static __used noinline
const struct cpumask *scx_bpf_task_cpumask(struct task_struct *p)
{
	return p->cpus_ptr;
}
...
int scx_bpf_pick_idle_cpu_untyped(unsigned long cpus_allowed)
{
 ...
 */
static noinline __used
int scx_bpf_has_idle_cpus_among_untyped(unsigned long cpus_allowed)
{
 ...
 */
static __used noinline
int scx_bpf_cpumask_test_cpu(int cpu, const struct cpumask *cpumask)
{
 ...
 */
static noinline __used
int scx_bpf_cpumask_first(const struct cpumask *cpus_allowed)
{
 ...
 */
static noinline __used
int scx_bpf_cpumask_first_untyped(unsigned long cpus_allowed)
{
