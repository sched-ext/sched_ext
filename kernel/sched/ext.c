...
/**
 * udsq_next_task - Iterate to the next task in a user dsq
 ...
 */
static struct task_struct *udsq_next_task(struct scx_dispatch_q *dsq,
...
#define udsq_for_each_task(p, dsq)						\
	for ((p) = udsq_next_task((dsq), NULL, false); (p);			\
	     (p) = udsq_next_task((dsq), (p), false))
