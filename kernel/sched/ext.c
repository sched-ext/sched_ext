 ...
 * Copyright (c) 2022 David Vernet <void@manifault.com>
 */
...
/*
 * During exit, a task may schedule after losing its PIDs and SCX needs to be
 * able to iterate tasks in all states. Maintain a dedicated task list which
 * contains every task between its fork and eventual free.
 */
...
static void scx_bpf_dispatch(struct task_struct *p, u64 dsq_id, u64 enq_flags);
...
static int scx_ops_enable(struct sched_ext_ops *ops)
{
	...
	/*
	 * Enable ops for each task. Fork is excluded by scx_fork_rwsem, so we
	...
	/* we're fully committed and can't fail */
 ...
 * @unsafe_data: format string parameters packaged using ___bpf_fill() macro
 * @data_len: @unsafe_data len
 *
 * Indicate that the BPF scheduler encountered a fatal error and initiate ops
 * disabling.
 */
void scx_bpf_error_bstr(unsigned long unsafe_fmt, unsigned long unsafe_data,
			u32 data_len)
{
	...
	ret = strncpy_from_kernel_nofault(bufs->fmt, (void *)unsafe_fmt,
					  sizeof(bufs->fmt));
	if (ret < 0) {
		scx_ops_error("failed to read the format string (%d)", ret);
		goto out_restore;
	}

	if (data_len % 8 || data_len > MAX_BPRINTF_VARARGS * 8 ||
	    (data_len && !unsafe_data)) {
		scx_ops_error("invalid data=%p and data_len=%u",
			      (void *)unsafe_data, data_len);
		goto out_restore;
	}

	ret = copy_from_kernel_nofault(bufs->data, (void *)unsafe_data, data_len);
	if (ret) {
		scx_ops_error("failed to read data fields (%d)", ret);
		goto out_restore;
	}

	ret = bpf_bprintf_prepare(bufs->fmt, UINT_MAX, bufs->data, &bin_args,
				  data_len / 8);
	if (ret < 0) {
	...
	ret = bstr_printf(bufs->msg, sizeof(bufs->msg), bufs->fmt, bin_args);
	bpf_bprintf_cleanup();
	if (ret < 0) {
		scx_ops_error("scx_ops_error(\"%s\", %p, %u) failed to format",
			      bufs->fmt, (void *)unsafe_data, data_len);
		goto out_restore;
	}
