...
extern int scx_bpf_dispatch(struct task_struct *p, s64 dq_id) __ksym;
extern int scx_bpf_dispatch_pid(s32 pid, s64 dq_id, u64 slice) __ksym;
extern int scx_bpf_pick_idle_cpu(const cpumask_t *cpus_allowed) __ksym;
...
static int dispatch_cpu_drain(struct bpf_dynptr *dynptr, void *context)
{
	...
	if (err) {
		/* Should never happen. */
		bpf_printk("Failed to read dispatch dynptr: %d, forcing exit\n", err);
		goto exit_fail;
	}
	...
	if (!p) {
		bpf_printk("Failed to find task for pid %d\n", pid);
		goto exit_fail;
	}
	...
	case RHONE_BPF_USER_MSG_TASK_UNKNOWN:
		...
		break;
	default:
		bpf_printk("Unknown user message type: %d\n", msg.msg_type);
		goto exit_fail;
	}
...
static int send_need_tasks_msg(s32 cpu)
{
	...
	if (!kprod_ringbuf) {
		/* Should never happen. */
		bpf_printk("Failed to get kprod_ringbuf for %d\n", cpu);
		return -ENOENT;
	}

	msg = bpf_ringbuf_reserve(kprod_ringbuf, sizeof(*msg), 0);
	if (!msg) {
		bpf_printk("Couldn't reserve ringbuf consume msg for %d\n", cpu);
		__sync_fetch_and_add(&nr_overflowed, 1);
		return -ENODATA;
	}
...
int BPF_STRUCT_OPS(rhone_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	if (num_drained < 0) {
		...
		if (ret < 0) {
			...
			bpf_printk("Couldn't send need tasks msg: %d\n", ret);
...
void BPF_STRUCT_OPS(rhone_consume, s32 cpu)
{
	...
	if (!dq_id) {
		/* Should never happen. */
		bpf_printk("Failed to get dq_id for cpu %d\n", cpu);
		exiting = true;
		return;
	}
	...
	/* Also always try to drain the global DQ. */
...
int BPF_STRUCT_OPS(rhone_init)
{
	...
	if (ret < 0) {
		/* Should never happen. */
		bpf_printk("Failed to create dispatch queues: %d\n", ret);
		return ret;
	}
