...
// TODO: Provide proper support in libbpf-rs
struct StructOps {
    _link: libbpf_rs::Link,
}

// HACK: This is just a copy from libbpf-rs so we can access the inner pointer
struct Map {
    _fd: i32,
    _name: String,
    _ty: libbpf_rs::libbpf_sys::bpf_map_type,
    _key_size: u32,
    _value_size: u32,

    // The ptr will be null if we use Map::create to create the map from the userspace side directly.
    ptr: *mut libbpf_rs::libbpf_sys::bpf_map,
}

impl StructOps {
    // We take a raw ptr here to avoid pinning the map reference. The caller
    // must ensure that the StructOps does not outlive the skel/map but doing
    // this with a reference would result in it being impossible to access maps
    // mutably while the structops is attached. Ideally this needs to be a
    // proper part of the Links struct.
    pub unsafe fn attach(map: *const libbpf_rs::Map) -> anyhow::Result<StructOps> {
        if (*map).map_type() != libbpf_rs::MapType::StructOps {
            bail!("Cannot attach a map unless it is of type StructOps");
        }
        let link = {
            let m = std::mem::transmute::<*const libbpf_rs::Map, *const Map>(map);
            let p = libbpf_rs::libbpf_sys::bpf_map__attach_struct_ops((*m).ptr);
            let rc = libbpf_rs::libbpf_sys::libbpf_get_error(p as *const std::ffi::c_void);
            if rc != 0 {
                bail!(std::io::Error::from_raw_os_error(rc as i32));
            }
            p
        };
        Ok(StructOps {
            _link: libbpf_rs::Link::from_ptr(link),
        })
    }
}

// This is defined in atropos.h, make sure if you change it here
// it changes there and vice versa
const MAX_DOMS: usize = 64;
...
fn parse_cpusets(cpumasks: &[String]) -> anyhow::Result<CpusetDqPair> {
    if cpumasks.len() > MAX_DOMS {
        bail!(
            "Number of requests DSQs ({}) is greater than MAX_DOMS ({})",
            cpumasks.len(),
            MAX_DOMS
        );
    }
    ...
    let mut cpusets = vec![bitvec![u64, Lsb0; 0; MAX_CPUS]; cpumasks.len()];
    ...
    for (cpu, &dq) in cpus.iter().enumerate() {
        if dq < 0 {
            bail!("Cpu {} not assigned to any dq", cpu);
        }
    }
...
pub fn run(
    ...
    let _structops = unsafe {
        StructOps::attach(skel.maps().atropos()).context("Failed to attach atropos struct ops")?
    };
