...
struct Opts {
    ...
    #[clap(short, long, num_args = 1.., conflicts_with = "cache_level")]
    cpumasks: Vec<String>,
...
struct LoadBalancer<'a, 'b> {
    maps: AtroposMapsMut<'a>,
    prev_runnable_for: &'b mut BTreeMap<i32, u64>,
    nr_doms: usize,

    pids_by_load: Vec<BTreeMap<OrderedFloat<f64>, PidInfo>>,
    global_load: f64,
    dom_loads: Vec<f64>,
...
impl<'a, 'b> LoadBalancer<'a, 'b> {
    ...
    fn new(
        ...
        Self {
            ...
            pids_by_load: (0..nr_doms).map(|_| BTreeMap::<_, _>::new()).collect(),
            global_load: 0f64,
    ...
    fn read_task_loads(&mut self, period: Duration) -> Result<()> {
        ...
        let mut runnable_for = BTreeMap::<i32, u64>::new();
        ...
        for key in task_data.keys() {
                ...
                .context("Failed to lookup task_data")?
            {
                ...
                let mut delta = match self.prev_runnable_for.get(&pid) {
                    Some(prev_for) => this_for - prev_for,
                    None => this_for,
                };

                // Non-zero this_at indicates that the task is currently
                // runnable. Note that we read runnable_at and runnable_for
                // without any synchronization and there is a small window where
                // we end up misaccounting. While this can cause temporary
                // error, it's unlikely to cause any noticeable misbehavior
                // especially given the load value clamping.
                ...
                runnable_for.insert(pid, this_for);

                let pid_load = (weight as f64 * delta as f64 / period.as_nanos() as f64)
                    .clamp(0.0, weight as f64);

                global_load_sum += pid_load;
                self.dom_loads[task_ctx.dom_id as usize] += pid_load;
                // Only record pids that are eligible for load balancing
                if task_ctx.dom_mask == (1u64 << task_ctx.dom_id) {
                    continue;
                }
                self.pids_by_load[task_ctx.dom_id as usize].insert(
                    OrderedFloat(pid_load),
                    PidInfo {
                        pid,
        ...
        self.global_load = global_load_sum / self.nr_doms as f64;
        *self.prev_runnable_for = runnable_for;
        Ok(())
    }
    ...
    fn calculate_dom_load_balance(&mut self) -> Result<()> {
        for (dom, dom_load) in self.dom_loads.iter().enumerate() {
            let imbal = dom_load - self.global_load;
            if imbal.abs() >= self.global_load * Self::LOAD_IMBAL_HIGH_RATIO {
                if imbal > 0f64 {
    ...
    // can run in @pull_dom.
    fn find_first_candidate<'c, I>(pids_by_load: I, pull_dom: u32) -> Option<(f64, &'c PidInfo)>
    where
        I: IntoIterator<Item = (&'c OrderedFloat<f64>, &'c PidInfo)>,
    {
        match pids_by_load
            .into_iter()
            .skip_while(|(_, pi)| pi.migrated.get() || pi.dom_mask & (1 << pull_dom) == 0)
            .next()
        {
            Some((OrderedFloat(load), pi)) => Some((*load, pi)),
            None => None,
        }
    }
    ...
    fn pick_victim(
        ...
        trace!(
            "to_xfer={:.2} pids_by_load={:?}",
            to_xfer,
            &self.pids_by_load[push_dom as usize]
        );
	...
	// scanning right and picking the better of the two.
        let (load, pi, new_imbal) = match (
            ...
            (Some((load, pi)), None) | (None, Some((load, pi))) => (load, pi, calc_new_imbal(load)),
            (Some((load0, pi0)), Some((load1, pi1))) => {
                let (new_imbal0, new_imbal1) = (calc_new_imbal(load0), calc_new_imbal(load1));
                if new_imbal0 <= new_imbal1 {
                    (load0, pi0, new_imbal0)
                } else {
                    (load1, pi1, new_imbal1)
                }
            }
        };

        // If the best candidate can't reduce the imbalance,
        // there's nothing to do for this pair.
        ...
        while let Some((OrderedFloat(mut to_push), push_dom)) = self.doms_to_push.pop_last() {
            ...
            loop {
                ...
                for (to_pull, pull_dom) in pull_doms.iter_mut() {
                    ...
                    {
                        // Execute migration.
                        pi.migrated.set(true);
                        to_push -= load;
                        *to_pull -= load;
                        pushed += load;

                        // Ask BPF code to execute the migration.
                        let cpid = (pi.pid as libc::pid_t).to_ne_bytes();
                        self.maps
                            .lb_data()
                            .update(
                                &cpid,
                            ...
                            .context("Failed to update lb_data map")?;
...
struct Scheduler<'a> {
    ...
    prev_runnable_for: BTreeMap<i32, u64>, // (PID, nsecs)
}
...
impl<'a> Scheduler<'a> {
    ...
    fn cpusets_from_cache(
        ...
        for cpu in 0..nr_cpus {
            ...
            if id >= atropos_sys::MAX_DOMS {
                bail!(
                    "CPU {} has level {} cache ID of {} which is greater than MAX_DOMS ({})",
                    cpu,
                    level,
                    id,
                    atropos_sys::MAX_DOMS
        ...
        if nr_not_found > 1 {
            warn!(
                "Couldn't determine level {} cache IDs for {} CPUs out of {}, assigneded to dom0",
                level, nr_not_found, nr_cpus
            );
        }

        let mut cpusets =
            vec![bitvec![u64, Lsb0; 0; atropos_sys::MAX_CPUS as usize]; max_id as usize + 1];
        for cpu in 0..nr_cpus {
            cpusets[cpus[cpu] as usize].set(cpu, true);
        }
    ...
    fn init(opts: &Opts) -> Result<Self> {
        ...
        for (dom, cpuset) in cpusets.iter().enumerate() {
            ...
            info!("dom {} cpumask{}", dom, &cpumask_str);
        }
        ...
        Ok(Self {
            ...
            prev_total_cpu,
            prev_runnable_for: BTreeMap::new(),
        })
    }
    ...
    fn report(
        ...
        info!(
            "cpu={:6.1} load={:7.1} balances={} task_get_errs={} processing={:?}ms",
            cpu_busy * 100.0,
            global_load,
    ...
    fn step(&mut self) -> Result<()> {
        ...
        // mutable borrows.
        let (global_load, dom_loads, imbal) = (lb.global_load, lb.dom_loads, lb.imbal);
