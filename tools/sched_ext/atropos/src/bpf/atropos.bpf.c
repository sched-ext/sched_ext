...
extern int scx_bpf_cpumask_test_cpu(int cpu,
				    const struct cpumask *cpumask) __ksym;
extern unsigned int scx_bpf_cpumask_first_untyped(unsigned long cpumask) __ksym;
extern bool
scx_bpf_has_idle_cpus_among_untyped(unsigned long cpus_allowed) __ksym;
extern bool scx_bpf_test_and_clear_cpu_idle(s32 cpu) __ksym;
extern s32 scx_bpf_dq_nr_queued(s64 dq_id) __ksym;
extern int scx_bpf_create_dq(s64 dq_id, int node) __ksym;
extern bool scx_bpf_consume(s64 dq_id) __ksym;
extern int scx_bpf_pick_idle_cpu_untyped(unsigned long cpus_allowed) __ksym;
extern u64 scx_bpf_task_cpu(const struct task_struct *p) __ksym;
...
s64 BPF_STRUCT_OPS(atropos_enqueue, struct task_struct *p, u32 enq_flags)
{
...
void BPF_STRUCT_OPS(atropos_enable, struct task_struct *p)
{
	...
	task_set_domain(&task_ctx, p, scx_bpf_task_cpumask(p));
	pid_t pid = p->pid;
	long ret =
