...
void BPF_STRUCT_OPS(qmap_cpu_acquire, s32 cpu, struct scx_cpu_acquire_args *args)
{
	bpf_printk("CPU %d reacquired", cpu);
}

void BPF_STRUCT_OPS(qmap_cpu_release, s32 cpu, struct scx_cpu_release_args *args)
{
	bpf_printk("Draining CPU %d, preempted by %s", cpu, args->task->comm);
...
struct sched_ext_ops qmap_ops = {
	...
	.cpu_acquire		= (void *)qmap_cpu_acquire,
