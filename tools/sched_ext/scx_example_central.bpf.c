...
/*
 * This global FIFO scheduler demonstrates the followings:
 *
 ...
 *    dispatching from the global queue to each CPU's local dq from dispatch().
 ...
 *    ticks on CONFIG_NO_HZ_FULL kernels running w/ the proper nohz_full
 ...
 * c. Preemption
 *
 *    Kthreads are unconditionally queued to the head of a matching local dq and
 *    dispatched with SCX_DQ_PREEMPT. This ensures that a kthread is always
...
s64 BPF_STRUCT_OPS(central_enqueue, struct task_struct *p, u64 enq_flags)
{
	s32 pid = p->pid;
	...
	/*
	 * Push per-cpu kthreads at the head of local dq's and preempt the
	...
	if ((p->flags & PF_KTHREAD) && p->nr_cpus_allowed == 1) {
		...
		return SCX_DQ_LOCAL | SCX_DQ_PREEMPT;
	}

	if (bpf_map_push_elem(&central_q, &pid, 0)) {
		__sync_fetch_and_add(&nr_overflows, 1);
		p->scx.slice = SCX_SLICE_INF;
		return FALLBACK_DQ_ID;
	}
...
static int dispatch_a_task_loopfn(u32 idx, void *data)
{
	 ...
	 * to the fallback dq.
	 */
	if (!scx_bpf_cpumask_test_cpu(cpu, scx_bpf_task_cpumask(p))) {
		__sync_fetch_and_add(&nr_mismatches, 1);
		p->scx.slice = SCX_SLICE_INF;
		scx_bpf_dispatch(p, FALLBACK_DQ_ID);
		return 0;
	}

	/* dispatch to the local dq and mark that @cpu doesn't need more tasks */
	p->scx.slice = SCX_SLICE_INF;
	scx_bpf_dispatch(p, SCX_DQ_LOCAL_ON | cpu);
...
s32 BPF_STRUCT_OPS(central_dispatch, s32 cpu, struct task_struct *prev)
{
	/* if out of tasks to run, gimme more */
	if (!scx_bpf_dq_nr_queued(FALLBACK_DQ_ID)) {
		bool *gimme = MEMBER_VPTR(cpu_gimme_task, [cpu]);
		if (gimme)
			*gimme = true;
	}
...
void BPF_STRUCT_OPS(central_consume, s32 cpu)
{
	 ...
	 * the fallback dq if central.
	 */
	if (cpu != central_cpu)
		scx_bpf_consume(FALLBACK_DQ_ID);
}
...
static int kick_cpus_loopfn(u32 idx, void *data)
{
	...
	if (scx_bpf_dq_nr_queued(FALLBACK_DQ_ID) ||
	    scx_bpf_dq_nr_queued(SCX_DQ_LOCAL_ON | cpu))
		;
	else if (*nr_to_kick)
		(*nr_to_kick)--;
	else
...
int BPF_STRUCT_OPS(central_init)
{
	...
	ret = scx_bpf_create_dq(FALLBACK_DQ_ID, -1);
	if (ret)
		return ret;
...
void BPF_STRUCT_OPS(central_exit, struct scx_ops_exit_info *ei)
{
