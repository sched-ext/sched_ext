...
enum scx_consts {
	...
	SCX_BASE_SLICE		= (20 * NSEC_PER_MSEC),
};
...
enum scx_ops_flags {
	 ...
	 * specified, such tasks are passed to ->enqueue() with
	 * %SCX_ENQ_LAST_TASK. See the comment above the flag for more info.
	 */
	SCX_OPS_ENQ_LAST_TASK	= 1LLU << 1,
...
struct sched_ext_ops {
	...
	/**
	 * set_group_weight - A cgroup's weight is being changed
	 * @cgrp: cgroup whose weight is being updated
	 * @weight: new weight [1..10000]
	 *
	 * Update @tg's weight to @weight.
	 */
	void (*set_group_weight)(struct cgroup *cgrp, u32 weight);
	 ...
	 */
	int (*prep_enable)(struct task_struct *p,
	 ...
	 * migration.
	 */
	int (*cgroup_prep_move)(struct task_struct *p,
