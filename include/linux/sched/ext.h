...
/*
 * DSQ IDs are 64bit of the format:
 *
 *   Bits: [63] [62 ..  0]
 *         [ B] [   ID   ]
 *
...
enum scx_ops_flags {
	 ...
	 * flag is specified, such tasks are passed to ->enqueue() with
...
struct sched_ext_ops {
	 ...
	 * the dsq's using scx_bpf_dispatch(). The maximum number of tasks which
	 ...
	 * caller should consult the %SCX_CPU_ flags in the release.
	 */
...
struct sched_ext_entity {
	...
	/*
	 * Runtime budget in nsecs. The BPF scheduler must set this to a
